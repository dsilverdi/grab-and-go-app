// Code taken from:
// https://github.com/android/camera-samples/blob/main/CameraXTfLite/utils/src/main/java/com/example/android/camera/utils/YuvToRgbConverter.kt
...
//    }
    ...
    fun getBytesBuffer(image: Image): ByteArray {
        pixelCount = image.cropRect.width() * image.cropRect.height()
        // Bits per pixel is an average for the whole image, so it's useful to compute the size
        // of the full buffer but should not be used to determine pixel offsets
        val pixelSizeBits = ImageFormat.getBitsPerPixel(ImageFormat.YUV_420_888)
        return ByteArray(pixelCount * pixelSizeBits / 8)
    }
